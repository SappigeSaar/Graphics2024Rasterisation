...
void main()
{
    ...
    vec4 directionLight = normalise(positionLight - positionWorld);

    vec4 refracted = directionLight - 2.0 * dot(directionLight, normalise(normalWordl)) * normalise(NormalWorld);

    outputColor = texture(diffuseTexture, uv) * intensity * 1.0 / 
            dot(positionLight - positionWorld, positionLight - positionWorld) * 
            (
                max(0.0, dot(normalise(normalWorld), directionLight)) 
                + pow( max(0.0, dot(normalise(refracted), normalise(cameraWorldPosition - positionWorld))) , 10.0) 
            );
